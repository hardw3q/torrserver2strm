...
### Без очистки удаленных торрентов

По умолчанию cleanup включен. Чтобы отключить:
...
| `--cleanup` | Нет | Удалять `.strm` файлы удаленных торрентов (включено по умолчанию) | `True` |
| `--no-cleanup` | Нет | Отключить автоматическое удаление `.strm` файлов удаленных торрентов | - |
...
7. **Автоматическая очистка**: По умолчанию скрипт автоматически удаляет `.strm` файлы торрентов, которые были удалены из TorrServer, а также пустые директории.
